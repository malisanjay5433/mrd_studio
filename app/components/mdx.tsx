...
const components = {
	...
	a: ({ className, ...props }) => (
		<Link
			className={clsx(
				"font-medium text-brand-terracotta underline decoration-brand-terracotta/40 underline-offset-4 hover:text-black",
				className,
			)}
			{...props}
		/>
	...
	img: ({
		...
		<img
			className={clsx(
				"h-auto max-w-full rounded-md border border-brand-rule",
				className,
			)}
			alt={alt}
			{...props}
	...
	hr: ({ ...props }) => <hr className="my-4 border-brand-rule md:my-8" {...props} />,
	...
	tr: ({ className, ...props }: React.HTMLAttributes<HTMLTableRowElement>) => (
		<tr
			className={clsx(
				"m-0 border-t border-brand-rule p-0 even:bg-brand-sand/12",
				className,
			)}
			{...props}
		/>
